...
(function() {
    ...
    async function init() {
        ...
        try {
            ...
            populateIceCreamList(flavors) // name of the function
        }
        catch(err) {
            showErrorMessage(err.message); // name of the function
        }
    }
    ...
    //async because fetching data from server
    ...
    function updateResult(data) {
        if(!data) {
            ...
            if(data.image && data.image.length > 0) { // if no iamage, will not show a "broken image", but is empty
